...
void Object::loadMultiviewShaderFromAsset(const char * vpath, const char * fpath) {
    mMultiviewShader = Shader::findShader(vpath, fpath);
    if (mMultiviewShader != NULL)
        return;
    ...
    mMultiviewShader = std::make_shared<Shader>(mName, vpath, vstr, fpath, fstr);
    bool ret = mMultiviewShader->compile();
    ...
    Shader::putShader(mMultiviewShader);
}
...
void Object::drawMultiview(const Matrix4 projection[2], const Matrix4 eye[2], const Matrix4& view, const Vector4& lightDir) {
}
