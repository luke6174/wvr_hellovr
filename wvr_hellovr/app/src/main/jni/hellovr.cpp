...
#define OBJ_ERROR_CHECK(obj) if (obj->hasError() || obj->hasGLError()) return false
	...
	if (gMultiview == false) {
    	...
    	for (int i = 0; i < WVR_GetTextureQueueLength(mLeftEyeQ); i++) {
        	...
        	fbo = new FrameBufferObject((int)WVR_GetTexture(mLeftEyeQ, i).id, mRenderWidth, mRenderHeight, true);
        	if (!fbo || fbo->hasError()) return false;
        	mLeftEyeFBOMSAA.push_back(fbo);

        	fbo = new FrameBufferObject((int)WVR_GetTexture(mLeftEyeQ, i).id, mRenderWidth, mRenderHeight);
        	if (!fbo || fbo->hasError()) return false;
    	...
    	for (int i = 0; i < WVR_GetTextureQueueLength(mRightEyeQ); i++) {
        	...
        	fbo = new FrameBufferObject((int)WVR_GetTexture(mRightEyeQ, i).id, mRenderWidth, mRenderHeight, true);
        	if (!fbo || fbo->hasError()) return false;
        	mRightEyeFBOMSAA.push_back(fbo);

        	fbo = new FrameBufferObject((int)WVR_GetTexture(mRightEyeQ, i).id, mRenderWidth, mRenderHeight);
        	if (!fbo || fbo->hasError()) return false;
        	mRightEyeFBO.push_back(fbo);
		}
    } else { /* Init MultiView FBO */
		mMultiviewQ = WVR_ObtainTextureQueue(WVR_TextureTarget_2D_ARRAY, WVR_TextureFormat_RGBA, WVR_TextureType_UnsignedByte, mRenderWidth, mRenderHeight, 0);

    	for (int i = 0; i < WVR_GetTextureQueueLength(mMultiviewQ); i++) {
        	FrameBufferObject* fbo;

        	fbo = new FrameBufferObject((int)WVR_GetTexture(mMultiviewQ, i).id, mRenderWidth, mRenderHeight, true, true);
        	if (!fbo || fbo->hasError()) return false;
        	mMultiviewFBOMSAA.push_back(fbo);

        	fbo = new FrameBufferObject((int)WVR_GetTexture(mMultiviewQ, i).id, mRenderWidth, mRenderHeight, false, true);
        	if (!fbo || fbo->hasError()) return false;
        	mMultiviewFBO.push_back(fbo);
    	}
	}
...
void MainApplication::shutdownGL() {
    ...
    if (mMultiviewQ != 0) {
        for (int i = 0; i < WVR_GetTextureQueueLength(mMultiviewQ); i++) {
            delete mMultiviewFBOMSAA.at(i);
            delete mMultiviewFBO.at(i);
        }
        WVR_ReleaseTextureQueue(mMultiviewQ);
    }
...
public:
    WVR_AnalogState_t mAnalogState[3];

    ControllerState() : mLastTouchpadPressed(false), mTouchpadClicked(false) {
    }
        ...
        if (WVR_IsDeviceConnected(controllerArray[idx])) {
            ...
            if (WVR_GetInputDeviceState(controllerArray[idx], inputType, &buttons, &touches, cs.mAnalogState, analogCount)) {
                cs.updateState(controllerArray[idx]);
                    if ( controllerArray[idx]==WVR_DeviceType_Controller_Right && mPointToSphere_R){
                        if (cs.touchpadClicked() &&  mCurFocusController==WVR_DeviceType_Controller_Right) {
                            moveSphereHandler();
                        }
                    }else if(controllerArray[idx]==WVR_DeviceType_Controller_Left && mPointToSphere_L){
...
bool MainApplication::renderFrame() {
	...
	if (gMultiview == false) {
		...
		mIndexMultiview = WVR_GetAvailableTextureIndex(mMultiviewQ);
	}
	...
	if (gMultiview == false) {
    	...
    	WVR_TextureParams_t multiviewEyeTexture = WVR_GetTexture(mMultiviewQ, mIndexMultiview);
...
void MainApplication::renderStereoTargets() {
	...
	if (gMultiview == false) {
    	...
    	fbo->bindFrameBuffer(gMultiview);
    	fbo->glViewportFull();
    	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    	renderScene(WVR_Eye_Left);
    	fbo->unbindFrameBuffer(gMultiview);
    	...
    	fbo = gMsaa ? mMultiviewFBOMSAA.at(mIndexMultiview) : mMultiviewFBO.at(mIndexMultiview);
...
void MainApplication::renderSceneMultiview() {
    ...
    // [0] Left, [1] Right
    Matrix4 multiviewProjection[2];
    Matrix4 multiviewEyePos[2];
    multiviewProjection[0] = mProjectionLeft;
    multiviewProjection[1] = mProjectionRight;
    multiviewEyePos[0] = mEyePosLeft;
    multiviewEyePos[1] = mEyePosRight;



    if (mGridPicture || mGridPicture->isEnabled()) {
		mGridPicture->drawMultiview(multiviewProjection, multiviewEyePos, mHMDPose, mLightDir);
     	return;
    }

    // SkyBox
    if (mSkyBox) {
        mSkyBox->drawMultiview(multiviewProjection, multiviewEyePos, mHMDPose, mLightDir);
    }
    ...
    if (!isInputCapturedBySystem) {
        ...
        if (mControllerAxes) {
            mControllerAxes->drawMultiview(multiviewProjection, multiviewEyePos, view, mLightDir);
        }
    }
    ...
    for (uint32_t id = WVR_DEVICE_HMD + 1;
		...
		ControllerCube->drawMultiview(multiviewProjection, multiviewEyePos, view, light);
    }


    if (mSphere) {
        mSphere->drawMultiview(multiviewProjection, multiviewEyePos, mHMDPose, mLightDir);
    }

    if (mFloor) {
        mFloor->drawMultiview(multiviewProjection, multiviewEyePos, mHMDPose, mLightDir);
    }
