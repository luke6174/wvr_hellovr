...
SkyBox::SkyBox(bool debug) : Object() {
    ...
    loadMultiviewShaderFromAsset("shader/vertex/skybox_multiview_vertex.glsl", "shader/fragment/skybox_fragment.glsl");
    if (mHasError)
        return;
    mMultiviewMatrixLocation = mMultiviewShader->getUniformLocation("matrix");
    mMultiviewTextureLocation = mMultiviewShader->getUniformLocation("atexture");
...
void SkyBox::drawMultiview(const Matrix4 projection[2], const Matrix4 eye[2], const Matrix4& view, const Vector4& lightDir) {
    if (!mEnable)
        return;
    ...
    // Skybox didn't need eye.
    Matrix4 matrix[2];
    matrix[0] = projection[0] * viewClone;
    matrix[1] = projection[1] * viewClone;
    ...
    glUniformMatrix4fv(mMultiviewMatrixLocation, 2, GL_FALSE, float_array);
    glActiveTexture(GL_TEXTURE0);
    mTexture->bindTextureCubeMap();
    glUniform1i(mMultiviewTextureLocation, 0);
    mVAO->bindVAO();
    glDrawArrays(GL_TRIANGLES, 0, mVertices);
