...
ControllerAxes::ControllerAxes() : Object() {
    ...
    loadMultiviewShaderFromAsset("shader/vertex/vcm_vertex.glsl", "shader/fragment/c_fragment.glsl");
    if (mHasError)
        return;
    mMultiviewMatrix = mMultiviewShader->getUniformLocation("matrix");
...
void ControllerAxes::drawMultiview(const Matrix4 projection[2], const Matrix4 eye[2], const Matrix4& view, const Vector4& lightDir) {
    if (!mEnable || !mVAO)
        return;

    Matrix4 matrix[2];
    matrix[0] = projection[0] * eye[0] * view;
    matrix[1] = projection[1] * eye[1] * view;
    ...
    glUniformMatrix4fv(mMultiviewMatrix, 2, false, float_array);
    mVAO->bindVAO();
    glDrawArrays(GL_TRIANGLES, 0, mVertCount);
