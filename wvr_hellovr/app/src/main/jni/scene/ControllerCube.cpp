...
ControllerCube::ControllerCube(WVR_DeviceType deviceType)
    ...
    loadMultiviewShaderFromAsset("shader/vertex/vtnm_vertex.glsl", "shader/fragment/ti_fragment.glsl");
    if (mHasError)
        return;
    mMultiviewMatrixLocation = mMultiviewShader->getUniformLocation("matrix");
    mMultiviewNormalMatrixLocation = mMultiviewShader->getUniformLocation("normalm");
    mMultiviewLightDirLocation = mMultiviewShader->getUniformLocation("l_dir");
...
void ControllerCube::drawMultiview(const Matrix4 projection[2], const Matrix4 eye[2], const Matrix4& view, const Vector4& lightDir) {
    if (!mEnable || mHasError || !mVAO || !mTexture)
        return;
    Matrix4 matrix[2];
    matrix[0] = projection[0] * eye[0] * view * getTransforms();
    matrix[1] = projection[1] * eye[1] * view * getTransforms();
    ...
    glUniformMatrix3fv(mMultiviewNormalMatrixLocation, 1, false, mNormalMatrix.get());
    glUniform4f(mMultiviewLightDirLocation, lightDir.x, lightDir.y, lightDir.z, lightDir.w);
    mVAO->bindVAO();
    glActiveTexture(GL_TEXTURE0);
    mTexture->bindTexture();
    glDrawElements(GL_TRIANGLES, mTrianglesX3, GL_UNSIGNED_INT, 0);
